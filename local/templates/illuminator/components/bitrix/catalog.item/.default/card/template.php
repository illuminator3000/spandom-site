...
<div class="product-card">
	<a href = "<?=$item['DETAIL_PAGE_URL']?>" class="product-card__image">
		<img src="<?=$item['PREVIEW_PICTURE']['SRC']?>" width = "375" height="375" loading="lazy" alt="<?=$item['NAME']?>">
	</a>
	<div class="product-card__content">
		<div class="product-card__content-top">
			<?if(isset($item['PROPERTIES']['TAG']) && !empty($item['PROPERTIES']['TAG']['VALUE'])):?>
				<?
					$class = "default-tab--sale";
					if($item['PROPERTIES']['TAG']['VALUE_XML_ID'] == 'new'){
						$class = "default-tab--new";
					};	
				?>
			<div class="default-tab product-card__tab <?=$class?>"><?=$item['PROPERTIES']['TAG']['VALUE']?></div>
			<?endif;?>
			<div class="product-card__favorite favorite-btn " data-id = "<?=$item['ID']?>">
		...
		<div class="product-card__content-bottom">
			<div class="product-card__info">
				<a href = "<?=$item['DETAIL_PAGE_URL']?>" class="product-card__title"><?=$item['NAME']?></a>
				<div class="product-card__price">
					<?=$price['PRINT_BASE_PRICE']?>/м2 <span>3216 ₽/м2</span>
				</div>
			</div>
			<div class="product-card__basket-add" data-id = "<?=$item['ID']?>" >
				<svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
					...
					<ellipse cx="9.14062" cy="20.5156" rx="1.82812" ry="1.82812" fill="#222222" />
					<ellipse cx="17.2656" cy="20.5156" rx="1.82812" ry="1.82812" fill="#222222" />
				</svg>
				<span></span>
			</div>
		</div>
